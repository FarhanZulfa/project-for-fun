...
class HabitStore {
  constructor() {
    ...
    this.notes = {}; // { "YYYY-MM-DD": "Daily reflection note" }
    this.listeners = [];
    this.init();
  }
  ...
  init() {
    ...
    if (storedNotes) {
      ...
      // Seed a couple of realistic reflection notes
      ...
      this.notes = {
        [todayStr]: 'Strong energy today, focused on deep work and consistency! 🚀',
        [yesterdayStr]: 'Morning 5km walk at sunrise. Felt rejuvenated.'
      };
      this.saveNotes();
    }
  }
  ...
  setNoteForDate(dateStr, noteText) {
    if (noteText && noteText.trim()) {
      this.notes[dateStr] = noteText.trim();
    } else {
  ...
  importBackup(jsonString) {
    try {
      ...
      if (data && Array.isArray(data.habits) && typeof data.logs === 'object') {
        ...
        this.logs = data.logs;
        this.notes = (data.notes && typeof data.notes === 'object') ? data.notes : {};
        this.saveHabits();
        this.saveLogs();
        this.saveNotes();
