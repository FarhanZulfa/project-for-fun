...
- **HTML5 & SVG** ΓÇö Clean, semantic structure with vector line icons (no emojis in UI controls).
- **Vanilla CSS (No frameworks)** ΓÇö Custom design tokens, glassmorphism tooltips, soft double-layer diffuse shadows, and responsive grid layouts.
- **Vanilla JavaScript (ES6+ Modules)** ΓÇö Object-oriented architecture with decoupled Store, Stats, and SVG Heatmap rendering engines.
- **Web Audio API** ΓÇö Real-time synthesized sine wave chimes for tactile feedback without audio asset loading.
